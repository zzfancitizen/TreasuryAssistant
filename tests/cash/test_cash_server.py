...
from app.cash.server import app
...
def test_cash_server_exposes_agent_card() -> None:
...
def test_cash_server_accepts_a2a_message_send() -> None:
    response = TestClient(app).post("/", json=message_send_payload("查余额"))
...
def test_cash_server_accepts_a2a_message_stream() -> None:
    with TestClient(app) as client:
        with client.stream("POST", "/", json=message_stream_payload("查余额")) as response:
