...
async def test_sequential_plan_passes_previous_results_generically() -> None:
    client = RecordingA2AClient()
    executor = PlanExecutor(registry=AgentRegistry.default_local(), a2a_client=client)
    plan = RoutePlan(
...
async def test_executor_context_is_budgeted_and_dependency_scoped() -> None:
    ...
    executor = PlanExecutor(registry=AgentRegistry.default_local(), a2a_client=client, max_context_tokens=120)
    plan = RoutePlan(
        intent="liquidity_analysis",
        confidence=0.8,
        execution_mode="sequential",
